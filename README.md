...
#
#después de cargados agregar los datos de la mezcla: ojo, revisar que los archivos csv definitivamente esten separados por coma y no por punto y coma, si no ya saben como "repararlo"
...
mix <- load_mix_data(file="consumidores_itch.csv",
...
source <- load_source_data(filename="recursos_itch.csv",
                           source_factors=NULL,
                           conc_dep=FALSE,
                           data_type="means", mix)
#cargar el archivo de discriminaci?n
discr <- load_discr_data(filename="discriminacion_itch.csv", mix)

#Crear la gr?fica 
plot_data (filename="isospace_plot", plot_save_pdf=FALSE,
          plot_save_png=FALSE, mix,source,discr)

#calcular ?rea, valor de toda la comunidad
...
###########################################################################
########################################################################
#OJO LO SIGUIENTE SOLO SE HACE SI EL COMANDO ANTERIOR DE PLOT PRIOR NO TE FUNCIONÓ
#
...
# crear el modelo
write_JAGS_model(
  filename = "general_itch_modelo.txt",
  resid_err = TRUE,
  process_err = TRUE,
  mix,
  source)
